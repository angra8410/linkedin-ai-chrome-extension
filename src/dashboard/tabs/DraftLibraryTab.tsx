...
export default function DraftLibraryTab({ onOpenInDraft }: Props) {
  ...
  const [analyticsPreparedId, setAnalyticsPreparedId] = useState<string | null>(null);
  ...
  const handleMoveLeft = async (draft: PostDraft) => {
    try {
      ...
      setAnalyticsPreparedId((current) =>
  ...
  const handleLogPerformance = (draft: PostDraft) => {
    const postTitle =
      draft.prompt?.trim() ||
      draft.content.slice(0, 80).replace(/\s+/g, " ").trim() ||
      "Untitled post";
    ...
    setAnalyticsSeed({
      ...
      postTitle,
      pillar: draft.pillar || "",
      postedAt: draft.postedAt ?? Date.now(),
      format: "insight",
      contentSnippet: draft.content.slice(0, 220),
    });

    setAnalyticsPreparedId(draft.id);
  };
  ...
  const getCardTimestampLabel = (draft: PostDraft) => {
    if (draft.status === "posted") {
      return `Posted · ${formatDateTime(draft.postedAt ?? draft.createdAt)}`;
    }

    return `Created · ${formatDate(draft.createdAt)}`;
  };
  ...
  const renderDraftCard = (draft: PostDraft) => {
    ...
    const analyticsPrepared = analyticsPreparedId === draft.id;
